...
#The job requires 100 compute nodes (should make sense because 10 iterations*10 partitions)
#SBATCH -N 100
 
#The job requires 1 task per node
#SBATCH --ntasks-per-node=1
 
#The maximum walltime of the job is a 20 hours (hopefully that's enough)
#SBATCH -t 20:00:00
...
result_file='data/result/result_ensemble.pkl'
result_plot_file='plots/result_ensemble.png'
...
do
  ...
  do
    ...
    srun python ./ensemble.py $partitioned_data_file $grid_search_file $ensemble_file $ensemble_score_file $partitions $iterations $best_proportion $used_proportion
    printf '\n'
...
# frequency bands
srun python ./integrate_scores.py $raw_ensemble_score_file $result_file "${time_windows}" "${frequency_bands}"
